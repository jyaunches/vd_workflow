...
- spec_dir: Path to the spec directory containing spec.md (and optionally tests.md)
...
/execute-wf:spec-simplify @specs/2025-10-31_cli-analyze-intraday-impact/

# Auto-apply mode - automatically applies safe recommendations
/execute-wf:spec-simplify @specs/2025-10-31_cli-analyze-intraday-impact/ --auto-apply
```
...
You can type `/execute-wf:spec-simplify @` and Claude will provide directory suggestions as you type.
...
Read the PATTERNS.md file to understand which recommendations align with project standards and can be auto-applied. If no PATTERNS.md is found, use conservative defaults (only auto-apply obvious YAGNI removals).
...
**Note:** Validation is now handled by `validation.md` (BDD scenarios) generated during the review phase, not inline in the spec.
...
1. **Read Spec Directory**: Read `<spec_dir>/spec.md` and `<spec_dir>/tests.md` (if exists)
...
**Note:** Validation is now a separate artifact (`validation.md`) generated by `/execute-wf:validation-plan`, not inline in specs.
...
**Note:** Validation is now handled separately via `validation.md` (BDD scenarios), not as inline spec phases.
